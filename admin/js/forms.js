...
$("#myModal").on("hidden", function() { $("#myModal").remove();}); $("#myModal").modal({"backdrop"  : "static","keyboard"  : true,"show":true                     
 });return false;} var form_ir = false;function regformhash(form, endereco, num, compl, infoprox, username, usernamesobre, cpf, confc,  email, password, confirmpwd, getlat, getlong, Notcadastro) { var TOTALERROS = $("#MENSAERRO");var parag = '';var erros = 0;if(distnaoatend === true){distancianaoatendida();return false;
} var camposobrigatoriosvazio = false; if (endereco.value == ''||num.value == '' ||username.value == ''|| usernamesobre.value == ''|| cpf.value == ''|| confc.value == ''||password.value == ''||confirmpwd.value == ''|| getlat == ''||getlong ==  ''||Notcadastro == true ) { parag += '<li> Erro: Há campos obrigatórios vazio </li> '; TOTALERROS.empty().append(parag);mostrarerro();erros++;camposobrigatoriosvazio = true; }if(camposobrigatoriosvazio === false && distnaoatend === false){
 var regex = /^[0-9-A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ \.\-\,]+$/gi;if (!regex.test(endereco.value) ) { parag += '<li> Erro: Verifique o endereço </li> ';
erros++; } var regex = /^[0-9.]+$/; if (!regex.test(num.value) ) { parag += '<li> Erro: de digitação no campo Número da residência </li> '; erros++;  
} if(compl.value.length > 0) { var regex = /^[0-9-A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ \.\-\ ]+$/; if (!regex.test(compl.value) ){parag += '<li> Erro: de digitação no campo Complemento da residência </li> '; erros++;}}if(infoprox.value.length > 0) { var regex = /^[0-9-A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ ]+$/;  if (!regex.test(infoprox.value) ) { parag += '<li> Erro: de digitação no campo próximo à: </li> ';erros++; } }var regex = /^[A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ]+$/; if (!regex.test(username.value) ) { parag += '<li> Erro: de digitação no campo nome usuário </li> '; erros++; } var regex = /^[A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ]+$/;if (!regex.test(usernamesobre.value) ) {  parag += '<li> Erro: de digitação no campo sobrenome do usuário </li> ';erros++;  
...
parag += '<li> Erro: de digitação no campo senha do usuário </li> ';erros++; }if (password.value != confirmpwd.value) { parag += '<li> Erro: ao verificar senha - confirme a senha </li> '; erros++; }var regex = /^[0-9.,\-\ ]+$/;if (!regex.test(getlat) ) { parag += '<li> Erro: endereço inválido </li> '; erros++;  
 }else{ } var regex = /^[0-9.,\-\ ]+$/; if (!regex.test(getlong)  ) { parag += '<li> Erro: endereço inválido </li> '; erros++;} if (Notcadastro == true) {  
parag += '<li> Erro: endereço inválido </li> ';erros++;}if(erros != 0 ){ parag += "<li> Erros:"+erros+" </li> "; TOTALERROS.empty().append(parag);mostrarerro();
}else if(erros === 0 ){ form_ir = form; form_ir.appendChild(latcriadaclie);latcriadaclie.name = "latcriadaclie";latcriadaclie.type = "hidden";latcriadaclie.value = getlat;form_ir.appendChild(longcriadaclie); longcriadaclie.name = "longcriadaclie"; longcriadaclie.type = "hidden";longcriadaclie.value = getlong;form_ir.appendChild(Notcadcriad);Notcadcriad.name = "Notcadcriad";Notcadcriad.type = "hidden"; Notcadcriad.value = Notcadastro;phptatudook(); }}} 
